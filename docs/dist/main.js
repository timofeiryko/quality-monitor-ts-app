...
function computeRisk(rows) {
    ...
    for (const r of rows) {
        ...
        const logistic = (x) => 1 / (1 + Math.exp(-2 * (x - 0.5)));
        r.p = logistic(q);
    }
}
/**
...
function normaliseRows(rawRows) {
    ...
    for (const row of rawRows) {
        ...
        const t = parseFloat((_a = o['temperature']) !== null && _a !== void 0 ? _a : o['температура']);
        const v = parseFloat((_b = o['vibration']) !== null && _b !== void 0 ? _b : o['вибрация']);
        const w = parseFloat((_c = o['wear']) !== null && _c !== void 0 ? _c : o['износ']);
        if (isNaN(t) || isNaN(v) || isNaN(w))
            continue;
        const defectStr = (_d = o['defect']) !== null && _d !== void 0 ? _d : o['брак'];
        const defect = defectStr !== undefined && defectStr !== '' ? Number(defectStr) : undefined;
        const machine = (_e = o['machine']) !== null && _e !== void 0 ? _e : o['станок'];
...
function App() {
    ...
    React.useEffect(() => {
        ...
        const thresholdAlerts = rows.filter(r => {
            var _a;
            return ((_a = r.p) !== null && _a !== void 0 ? _a : 0) > 0.7 || r.v > 0.25 || r.t > 28 || r.w > 60;
        });
        ...
        const pData = rows.map(r => { var _a; return ((_a = r.p) !== null && _a !== void 0 ? _a : 0); });
        const datasets = [
    ...
    const avgP = rows.reduce((sum, r) => { var _a; return sum + ((_a = r.p) !== null && _a !== void 0 ? _a : 0); }, 0) / (rows.length || 1);
    return React.createElement('div', { className: 'container' }, 
    // Heading
    React.createElement('h1', { className: 'title is-4' }, 'Мониторинг качества — демо'), 
    // File input
    React.createElement('div', { className: 'box' }, React.createElement('div', { className: 'file has-name is-fullwidth' }, React.createElement('label', { className: 'file-label' }, React.createElement('input', { className: 'file-input', type: 'file', accept: '.csv,.txt', onChange: handleFileChange }), React.createElement('span', { className: 'file-cta' }, React.createElement('span', { className: 'file-icon' }, React.createElement('i', { className: 'fas fa-upload' })), React.createElement('span', { className: 'file-label' }, 'Выберите CSV…')), React.createElement('span', { className: 'file-name' }, 'Загрузить данные'))), React.createElement('p', { className: 'help' }, 'Ожидаемые колонки: temperature/температура, vibration/вибрация, wear/износ, defect/брак (опционально), machine/станок (опционально)')), 
    // KPI cards
    React.createElement('div', { className: 'columns' }, React.createElement('div', { className: 'column' }, React.createElement('div', { className: 'box has-text-centered' }, React.createElement('p', { className: 'heading' }, 'Температура'), React.createElement('p', { className: 'title is-4' }, latest ? `${latest.t.toFixed(2)} °C` : '—'))), React.createElement('div', { className: 'column' }, React.createElement('div', { className: 'box has-text-centered' }, React.createElement('p', { className: 'heading' }, 'Вибрация'), React.createElement('p', { className: 'title is-4' }, latest ? `${latest.v.toFixed(3)} м/с²` : '—'))), React.createElement('div', { className: 'column' }, React.createElement('div', { className: 'box has-text-centered' }, React.createElement('p', { className: 'heading' }, 'Износ'), React.createElement('p', { className: 'title is-4' }, latest ? `${latest.w.toFixed(1)} %` : '—'))), React.createElement('div', { className: 'column' }, React.createElement('div', { className: 'box has-text-centered' }, React.createElement('p', { className: 'heading' }, 'Риск брака (p)'), React.createElement('p', { className: 'title is-4' }, rows.length ? `${(avgP * 100).toFixed(1)} %` : '—')))), 
    // Chart container
    React.createElement('div', { className: 'box', style: { height: '300px' } }, React.createElement('canvas', { ref: canvasRef })), 
    // Alerts section
    React.createElement('div', { className: 'box' }, React.createElement('h2', { className: 'title is-6' }, 'Экстренные уведомления'), alerts.length === 0 ? React.createElement('p', { className: 'has-text-grey' }, 'Нет критических событий') :
        React.createElement('ul', {}, alerts.slice().reverse().map((r, idx) => {
            var _a;
            return React.createElement('li', { key: idx }, `⚠️ t=${r.t.toFixed(2)}°C · v=${r.v.toFixed(3)} м/с² · w=${r.w.toFixed(1)}% · p=${((_a = r.p) !== null && _a !== void 0 ? _a : 0).toFixed(2)}`);
        }))));
}
